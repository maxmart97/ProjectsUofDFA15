    ...
    public Image edge() {
        ...
        for(int row=1; row<myHeight-1; row++) {
            for(int col=1; col<myWidth-1; col++) {
                //if(col==0 ){
                  //  Xpart = (-0.5 * (this.myPixels[col][row].getRed())) + (0.5 * (this.myPixels[col][row].getRed()));
                   // Ypart = (-0.5 * (this.myPixels[col+1][row].getRed())) + (0.5 * (this.myPixels[col+1][row].getRed()));
                   // intensity = (int) (Math.ceil(Math.sqrt((Xpart*Xpart) + (Ypart*Ypart))));
               // }
                //else if(col==(myWidth-1)) {
                   // Xpart = (-0.5 * (this.myPixels[col-1][row].getRed())) + (0.5 * (this.myPixels[col-1][row].getRed()));
                   // Ypart = (-0.5 * (this.myPixels[col][row].getRed())) + (0.5 * (this.myPixels[col][row].getRed()));
                    //intensity = (int) (Math.ceil(Math.sqrt((Xpart*Xpart) + (Ypart*Ypart))));
               // }
               // else {
                    Xpart = -0.5 * this.myPixels[row][col-1].getRed() + 0.5 * this.myPixels[row][col+1].getRed();
                    Ypart = -0.5 * this.myPixels[row-1][col].getRed() + 0.5 * this.myPixels[row+1][col].getRed();
                    intensity = (int) (Math.ceil(Math.sqrt((Xpart*Xpart) + (Ypart*Ypart))));
               // }
                copy.myPixels[row][col] = new Color(intensity, intensity, intensity);
            }
